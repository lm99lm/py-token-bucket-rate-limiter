...
> Design notes for **py-token-bucket-rate-limiter** — a runnable python example that A small, time-injectable token-bucket rate limiter — no Redis, no dependencies.
...
- **Feature code** — the actual task: tokenbucket.
